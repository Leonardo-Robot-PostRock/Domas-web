...
export default async function handler(req, res) {
  ...
  let user = await axios
    .post(`${process.env.AUTH_BASE_URL}/v1/user/check-mesa-username`, req.body, {
      headers: {
    ...
    .catch((error) => {
      if (error.response) {
        ...
        return {
          ...
          message: error.response.data.message ? error.response.data.message : 'Hubo un error al realizar la petición.'
        };
