...
export default async function handler (req, res) {
    ...
    let response = await axios.post(`${process.env.AUTH_BASE_URL}/v1/ticket/dumas/create`, req.body, {
        headers: {
        ...
        .catch(error => {
            ...
            return { status: error.response?.status || 400, error: true, message: error.response?.data?.message || 'Hubo un error al realizar la petición.' };
        });
