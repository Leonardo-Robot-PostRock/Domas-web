...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    let teams = await axios
      .get(`${process.env.AUTH_BASE_URL}/v1/calendar`, {
        headers: {
    ...
    let addDayOff = await axios
      .post(`${process.env.AUTH_BASE_URL}/v1/calendar/add`, data, {
        headers: {
          Cookie: `auth_service=${session}`
        }
      })
    ...
    let editDayOff = await axios
      .put(`${process.env.AUTH_BASE_URL}/v1/calendar/update/${data.id}`, data, {
        headers: {
          Cookie: `auth_service=${session}`
        }
      })
    ...
    let editTeam = await axios
      .patch(`${process.env.AUTH_BASE_URL}/v1/calendar/team/active`, data, {
        headers: {
          Cookie: `auth_service=${session}`
        }
      })
    ...
    let deleteDayOff = await axios
      .delete(`${process.env.AUTH_BASE_URL}/v1/calendar/delete/${date_id}`, {
        headers: {
          Cookie: `auth_service=${session}`
        }
      })
