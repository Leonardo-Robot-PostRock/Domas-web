...
export const tomorrowDatetime = (): dayjs.Dayjs => currentDatetime().add(1, 'day');
...
export const startOfDate = (date: ConfigType): dayjs.Dayjs => dayjs(date).startOf('day');

export const endOfDate = (date: ConfigType): dayjs.Dayjs => dayjs(date).endOf('day');

export const datetimeFormatted = (date: ConfigType, format: string): string => dayjs(date).format(format);

export const datetimeDiff = (date1: ConfigType, date2: ConfigType): number => dayjs(date1).diff(dayjs(date2), 'day');

dayjs.extend(relativeTime);

export const datetimeFromNow = (date: ConfigType): string => dayjs(date).fromNow();
